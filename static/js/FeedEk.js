    ...
    $.fn.FeedEk = function(opt) {
        ...
        $.ajax({
            url: 'https://ajax.googleapis.com/ajax/services/feed/' +
                'load?v=1.0&num=' + def.MaxCount + '&output=json&q=' +
                encodeURIComponent(def.FeedUrl) + '&hl=en&callback=?',
            dataType: 'json',
            success: function(data) {
                $('#' + id).empty();
                $.each(data.responseData.feed.entries, function(e, item) {
                    s += '<li><div class="itemTitle"><a href="' + item.link +

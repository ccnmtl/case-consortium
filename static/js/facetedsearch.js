;(function() {
    ...
    var defaults = {
        items: [{a: 2, b: 1, c: 2}, {a: 2, b: 2, c: 1}, {a: 1, b: 1, c: 1},
                {a: 3, b: 3, c: 1}],
        ...
        listItemTemplate: '<div class=facetitem id="<%= id %>"' +
            '><%= name %> <span class=facetitemcount>' +
            '(<%= count %>)</span></div>',
        bottomContainer: '<div class=bottomline></div>',
        orderSelector: '#order-by',
        orderByTemplate: '<div class=orderby><span class="orderby-title"' +
            '>Sort by: </span><ul><% _.each(options, function(value, key)' +
            '{ %>' +
            '<li class=orderbyitem id=orderby_<%= key %>>' +
            '<%= value %> </li> <% }); %></ul></div>',
        countTemplate: '<!--<div class=facettotalcount>Results:' +
            '(<%= count %>)</div>-->',
        deselectTemplate: '<div class=deselectstartover>' +
            'Deselect all filters</div>',
        resultTemplate: '<div class=facetresultbox><%= name %></div>',
        noResults: '<div class=results>Sorry, but no items match ' +
            'these criteria</div>',
    ...
    function initFacetCount() {
            ...
            _.each(settings.facets, function(facettitle, facet) {
                if ($.isArray(item[facet])) {
                    _.each(item[facet], function(facetitem) {
                        settings.facetStore[facet][facetitem] = settings
                            .facetStore[facet][facetitem] || {
                                count: 0, id: _.uniqueId('facet_')
                            };
                    });
                } else {
                    if (item[facet] !== undefined) {
                        settings.facetStore[facet][item[facet]] = settings
                            .facetStore[facet][item[facet]] || {
                                count: 0, id: _.uniqueId('facet_')};
                    }
                }
            });
        ...
        _.each(settings.facetStore, function(facet, facettitle) {
            ...
            if (settings.facetSortOption &&
                settings.facetSortOption[facettitle]) {
    ...
    function filter() {
        ...
        _.each(settings.state.filters, function(filters, facettitle) {
            _.each(settings.facetStore[facettitle], function(facet) {
                if (facet.count === 0 &&
                    settings.state.filters[facettitle].length) {
                    facet.count = '+';
                }
            });
        });
    ...
    function order() {
        if (settings.state.orderBy) {
            ...
            settings.currentResults = _.sortBy(
                settings.currentResults, function(item) {
    ...
    function toggleFilter(key, value) {
        ...
        if (_.indexOf(settings.state.filters[key], value) == -1) {
            ...
            settings.state.filters[key] = _.without(
                settings.state.filters[key], value);
    ...
    function createFacetUI() {
        ...
        _.each(settings.facets, function(facettitle, facet) {
            ...
            _.each(settings.facetStore[facet],
                   function(filter, filtername) {
                       var item = {id: filter.id, name: filtername,
                                   count: filter.count};
                       var filteritem  = $(itemtemplate(item));
                       if (_.indexOf(settings.state.filters[facet],
                                     filtername) >= 0) {
        ...
        $('.facetitem').click(function(event) {
            ...
            $(settings.facetSelector)
                .trigger('facetedsearchfacetclick', filter);
            order();
            updateFacetUI();
            updateResults();
        });
    ...
    function updateFacetUI() {
        ...
        _.each(settings.facetStore, function(facet, facetname) {
            ...
            _.each(facet, function(filter, filtername) {
                ...
                if (settings.state.filters[facetname] &&
                    _.indexOf(settings.state.filters[facetname],
                              filtername) >= 0) {
    ...
    function updateResults() {
        $(settings.resultSelector)
            .html(settings.currentResults.length === 0 ?
                  settings.noResults : '');
        showMoreResults();
    }
    ...
    function showMoreResults() {
        var showNowCount =
            ...
            Math.min(settings.currentResults
                     .length - settings.state.shownResults,
                     settings.paginationCount) : settings.currentResults.length;
        var itemHtml = '';
        var template = _.template(settings.resultTemplate);
        for (var i = settings.state.shownResults;
             i < settings.state.shownResults + showNowCount; i++) {
